// pages/admin/AdminMedicalRecords.js
import { useEffect, useState } from "react";
import Swal from "sweetalert2";
import Loading from "../../components/Loading";
import { getAdminMedicalRecords, deleteMedicalRecordAdmin } from "../../services/adminService";
...
function AdminMedicalRecords() {
  ...
  const fetchRecords = async () => {
    try {
      setLoading(true);
      const response = await getAdminMedicalRecords(page, search);
      setRecords(response.data);
      setPagination(response.pagination);
    } catch (error) {
      console.error(error);
      toast.error("Gagal memuat data medical records");
    } finally {
      setLoading(false);
    }
  };

  const handleDelete = async (id, diagnosis, pet_name) => {
    const result = await Swal.fire({
      title: "Konfirmasi Hapus",
      html: `Apakah Anda yakin ingin menghapus rekam medis <strong>${diagnosis}</strong> untuk <strong>${pet_name}</strong>?`,
      icon: "warning",
    ...
    try {
      await deleteMedicalRecordAdmin(id);
      ...
      <div className="p-6 pb-32">
        ...
        <div className="mb-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h1 className="text-2xl font-bold text-[#2F3E36]">Medical Records</h1>
              <p className="text-sm text-[#2F3E36]/60">Kelola seluruh riwayat medis pengguna</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-6">
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                placeholder="Cari berdasarkan diagnosis atau nama pet..."
                value={search}
        ...
        {loading ? (
                ...
                >
                  ...
                  <div className="p-5">
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        <h3 className="font-bold text-lg text-[#2F3E36] line-clamp-1">{record.diagnosis}</h3>
                        <div className="flex items-center gap-1 mt-1">
                          <Dog className="w-3.5 h-3.5 text-[#7A9D82]" />
                          <p className="text-sm text-gray-500">{record.pet_name}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2 text-sm mb-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium text-[#2F3E36]">{record.owner_name}</span>
                      </div>
                      <div className="flex items-center gap-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={() => handleDelete(record.id, record.diagnosis, record.pet_name)}
          ...
          <div className="text-center py-16 bg-white rounded-xl border border-gray-100">
            ...
            <h3 className="text-lg font-semibold text-[#2F3E36]">Tidak ada data medical record</h3>
            <p className="text-gray-400 text-sm mt-1">Belum ada riwayat medis yang tersedia</p>
          </div>
        )}
      </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full animate-fadeInUp" onClick={e => e.stopPropagation()}>
            <div className="bg-linear-to-r from-[#7A9D82] to-[#8AAD92] text-white rounded-t-2xl px-6 py-4">
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-xl font-bold">Detail Medical Record</h2>
                </div>
            ...
            <div className="p-6 space-y-3">
              <div className="flex justify-between py-2 border-b border-gray-100">
                ...
                <span className="font-medium text-[#2F3E36]">{selectedRecord.diagnosis}</span>
              </div>
              <div className="flex justify-between py-2 border-b border-gray-100">
                <span className="text-gray-500">Pet</span>
                <span className="text-[#2F3E36]">{selectedRecord.pet_name}</span>
              </div>
              <div className="flex justify-between py-2 border-b border-gray-100">
                <span className="text-gray-500">Pemilik</span>
                <span className="text-[#2F3E36]">{selectedRecord.owner_name}</span>
              </div>
              <div className="flex justify-between py-2 border-b border-gray-100">
                <span className="text-gray-500">Treatment</span>
                <span className="text-[#2F3E36]">{selectedRecord.treatment || "-"}</span>
              </div>
              <div className="flex justify-between py-2 border-b border-gray-100">
                <span className="text-gray-500">Tanggal Pemeriksaan</span>
                <span className="text-[#2F3E36]">{formatDate(selectedRecord.record_date)}</span>
              </div>
              <div className="pt-2">
